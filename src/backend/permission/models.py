...
class Role(Base, DateTimeMixin):
    ...
    users = relationship("User", secondary=users_roles, back_populates="roles")  # 多对多
    menus = relationship("Menu", secondary=roles_menus, back_populates="roles")  # 多对多
...
class Menu(Base, DateTimeMixin):
    ...
    roles = relationship("Role", secondary=roles_menus, back_populates="menus")
